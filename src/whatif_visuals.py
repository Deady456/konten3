...
def _search_bing_images(query: str) -> list[str]:
    url = f"https://www.bing.com/images/search?q={quote(query + ' cinematic 4k wallpaper')}&form=HDRSC3"
    try:
...
def _generate_pollinations_flux_whatif(prompt: str, out_path: Path, seed: int = None) -> bool:
    try:
        clean_p = re.sub(r'[^a-zA-Z0-9\s,.-]', '', prompt).strip()
        enhanced_prompt = f"{clean_p}, hypothetical speculative science scenario, epic cinematic visual, vivid colors, dramatic lighting, highly detailed 8k, photorealistic masterpiece, concept art style"
...
def _process_image_card(base_img_path: Path, out_path: Path, topic_label: str = "WHAT IF", w: int = 1080, h: int = 1920, is_hook: bool = False):
    try:
        im = Image.open(base_img_path).convert("RGBA")
    except Exception:
        im = Image.new("RGBA", (w, h), (20, 20, 40, 255))
    ...
    if is_hook:
        badge_y = 120
        draw.rectangle([(50, badge_y), (360, badge_y + 60)], fill=(255, 100, 0, 240))
...
def fetch_all(scenes: list[dict], out_dir: Path, words: list[dict] = None, voice_audio: Path = None) -> list[Path]:
    ...
    print(f"    [What-If Visuals] Generating dynamic sci-fi cuts for {len(scenes)} scenes ({total_audio_dur:.1f}s audio)...")
    ...
    for i, scene in enumerate(scenes):
        ...
        for sub_idx in range(num_subclips):
            ...
            img_ready = False
            prompt = queries[sub_idx % len(queries)] if queries else scene.get("text", "speculative science what if scenario")
            img_ready = _generate_pollinations_flux_whatif(prompt, raw_img_path, seed=clip_counter + 300)
            ...
            if img_ready:
                ...
                _process_image_card(raw_img_path, final_img_path, "WHAT IF", w, h, (i == 0 and sub_idx == 0))
        ...
        print(f"    scene {i+1}/{len(scenes)}: {total_scene_dur:.1f}s -> {num_subclips} what-if cuts generated")

    print(f"    [What-If Visuals] Ready: {len(all_clips)} dynamic clips covering full video duration.")
    return all_clips
